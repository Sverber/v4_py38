...
class RunCycleManager:
    ...
    def start_run(self, run, dataset) -> None:
        ...
        # Optimizers
        optimizer_G_A2B = torch.optim.Adam(self.net_G_A2B.parameters(), lr=run.learning_rate, betas=(0.5, 0.999))
        optimizer_G_B2A = torch.optim.Adam(self.net_G_A2B.parameters(), lr=run.learning_rate, betas=(0.5, 0.999))
        optimizer_D_A = torch.optim.Adam(self.net_D_A.parameters(), lr=run.learning_rate, betas=(0.5, 0.999))
        optimizer_D_B = torch.optim.Adam(self.net_D_B.parameters(), lr=run.learning_rate, betas=(0.5, 0.999))
        ...
        for epoch in range(0, run.num_epochs):
            ...
            for i, data in progress_bar:
                ...
                identity_image_A = self.net_G_B2A(real_image_A)
                loss_identity_A = identity_loss(identity_image_A, real_image_A) * 5.0

                # Identity loss of A2B
                # G_A2B(B) should equal B if real B is fed
                identity_image_B = self.net_G_A2B(real_image_B)
                loss_identity_B = identity_loss(identity_image_B, real_image_B) * 5.0

                # GAN loss: D_A(G_A(A))
                fake_image_A = self.net_G_B2A(real_image_B)
                fake_output_A = self.net_D_A(fake_image_A)
                loss_GAN_B2A = adversarial_loss(fake_output_A, real_label)

                # GAN loss: D_B(G_B(B))
                fake_image_B = self.net_G_A2B(real_image_A)
                fake_output_B = self.net_D_B(fake_image_B)
                loss_GAN_A2B = adversarial_loss(fake_output_B, real_label)

                # Cycle loss
                recovered_image_A = self.net_G_B2A(fake_image_B)
                loss_cycle_ABA = cycle_loss(recovered_image_A, real_image_A) * 10.0

                recovered_image_B = self.net_G_A2B(fake_image_A)
                ...
                # Real A image loss
                real_output_A = self.net_D_A(real_image_A)
                error_D_real_A = adversarial_loss(real_output_A, real_label)

                # Fake A image loss
                fake_image_A = fake_A_buffer.push_and_pop(fake_image_A)
                fake_output_A = self.net_D_A(fake_image_A.detach())
                ...
                # Real B image loss
                real_output_B = self.net_D_B(real_image_B)
                error_D_real_B = adversarial_loss(real_output_B, real_label)

                # Fake B image loss
                fake_image_B = fake_B_buffer.push_and_pop(fake_image_B)
                fake_output_B = self.net_D_B(fake_image_B.detach())
                ...
                # Generate output
                _fake_image_A = self.net_G_A2B(real_image_B)
                _fake_image_B = self.net_G_A2B(real_image_A)

                # Generate original image from generated (fake) output
                _fake_original_image_A = self.net_G_A2B(_fake_image_B)
                _fake_original_image_B = self.net_G_A2B(_fake_image_A)
                ...
                self.track_loss(
                    ...
                    fake_image_A,
                    fake_image_B,
                    fake_original_image_A,
                    fake_original_image_B,
                )

                """ (5) Save all generated network output and """

                def __save_realtime_output(_output_path=f"{self.DIR_OUTPUTS}/{RUN_PATH}") -> None:
                ...
                def __save_end_epoch_output(_output_path=f"{self.DIR_OUTPUTS}/{RUN_PATH}"):
                    ...
                    (
                        ...
                        f"{_output_path}/A/epochs/EP{epoch}___fake_sample_MSE.png",
                        f"{_output_path}/B/epochs/EP{epoch}___fake_sample_MSE.png",
                        f"{_output_path}/A/epochs/EP{epoch}___fake_original_MSE{self.avg_mse_loss_A:.3f}.png",
                        f"{_output_path}/B/epochs/EP{epoch}___fake_original_MSE{self.avg_mse_loss_B:.3f}.png",
                    )
                ...
                def __print_progress() -> None:
                    ...
                    progress_bar.set_description(
                        f"[{self.dataset_group.upper()}][{epoch + 1}/{run.num_epochs}][{i + 1}/{len(loader)}] "
                ...
                if i % self.SHOW_IMAGE_FREQ == 0:
                    __save_realtime_output(_output_path=f"{self.DIR_OUTPUTS}/{RUN_PATH}")

                # Save the network weights at the end of the epoch
                if i + 1 == len(loader) and epoch % self.SAVE_EPOCH_FREQ == 0:
                    __save_end_epoch_output(_output_path=f"{self.DIR_OUTPUTS}/{RUN_PATH}")
            ...
            # self.end_epoch(self.net_G_A2B, self.net_G_A2B, self.net_D_A, self.net_D_B)

        """ </end> for epoch in range(0, run.num_epochs): """

        # Save last check points, after every run
        torch.save(self.net_G_A2B.state_dict(), f"{self.DIR_WEIGHTS}/{RUN_PATH}/net_G_A2B.pth")
        torch.save(self.net_G_A2B.state_dict(), f"{self.DIR_WEIGHTS}/{RUN_PATH}/net_G_A2B.pth")
        torch.save(self.net_D_A.state_dict(), f"{self.DIR_WEIGHTS}/{RUN_PATH}/net_D_A.pth")
        torch.save(self.net_D_B.state_dict(), f"{self.DIR_WEIGHTS}/{RUN_PATH}/net_D_B.pth")
    ...
    def track_loss(
        ...
        self.cum_mse_loss_f_or_A += mse_loss_f_or_A
        self.cum_mse_loss_f_or_B += mse_loss_f_or_B
