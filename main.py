...
# NAME_DATASET = f"Test_Set"  # 2208 x 1242 resize by a factor 0.15 to (83, 147) | (y, x)
# NAME_DATASET = f"QuickDevelop" # 2208 x 1242 resize by a factor 0.15 to (83, 147) | (y, x)
NAME_DATASET = f"Test_GrayscaleRGB"  # 2208 x 1242 resize by a factor 0.15 to (83, 147) | (y, x)
...
IMAGE_SIZE = (49, 88)  # USASOL images factor /15
RATIO_CROP = 0.82
...
def train(dataset: StereoDisparityDataset):
    ...
    for run in RunCycleBuilder.get_runs(PARAMETERS):
        ...
        netG_A2B = Generator(in_channels=1, out_channels=1).to(run.device)  # GRAYSCALE stereo --> GRAYSCALE disparity
        netG_B2A = Generator(in_channels=1, out_channels=1).to(run.device)  # GRAYSCALE disparity --> GRAYSCALE stereo
        netD_A = Discriminator(in_channels=1, out_channels=1).to(run.device)  # GRAYSCALE stereo
